...
class MetaLoader:
    ...
    def __init__(
        self, loaders, accum_steps: int = 1, distributed: bool = False, device=None, num_iters=None
    ):
    ...
    def __len__(self):
        if self.num_iters is None:
            return sum(len(l) for l in self.name2loader.values())
        else:
            return self.num_iters

    def __iter__(self) -> Iterator[Tuple]:
        """this iterator will run indefinitely if num_iters is None"""
        task_id = None
