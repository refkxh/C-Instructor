...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        # self.ob_gate = torch.nn.Parameter(torch.zeros(1, self.n_local_heads, 1, 1))
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], adapter=None):
        ...
        #     ob_adapter_len = ob_adapter.shape[1]
        #     ob_adapter_v = self.wv(ob_adapter).view(bsz, ob_adapter_len, self.n_local_heads, self.head_dim)
        #     ob_adapter_v = ob_adapter_v.transpose(1, 2)

        #     ob_adapter_k = self.wk(ob_adapter).view(bsz, ob_adapter_len, self.n_local_heads, self.head_dim)
        #     ob_adapter_k = ob_adapter_k.transpose(1, 2)
        ...
        # if ob_adapter is not None:
        #     ob_adapter_scores = torch.matmul(xq, ob_adapter_k.transpose(2, 3)) / math.sqrt(self.head_dim)
        #     ob_attn_mask = ob_attn_mask.unsqueeze(1)[:, :, :ob_adapter_scores.shape[2]]
        #     ob_adapter_scores = ob_adapter_scores * ob_attn_mask
        #     ob_adapter_scores = F.softmax(ob_adapter_scores.float(), dim=-1).type_as(xq) * ob_attn_mask
        #     ob_adapter_scores = self.ob_gate.tanh() * ob_adapter_scores
        #     output = output + torch.matmul(ob_adapter_scores, ob_adapter_v)
